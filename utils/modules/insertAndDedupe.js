                    ...
                    lines.forEach((line, index) => {
                        collection.find({
                            ...
                            if (!docs.length) {
                                ...
                                // if (index === lines.length-1) { db.close() }
                            } else {
...
//
// NOTE: this logic fails in this condition:
// with these payees: "city of atlanta", "city of atlanta  utility"
// and the first not having a matches array, the city water transaction "city of atlanta  utility"
// would match on the first instead of the second. fixing this seems to be
// a pain in the ass since all payee matches would need to be found then compared
// to find the best match. lame.
function updatePayee (line, payees, categories) {
    for(let i=0; i<payees.length; i++) {
        ...
        if (payees[i].matches) {
            ...
            if (line.payee.toLowerCase().indexOf(payees[i].payee.toLowerCase()) > -1) {
                line.payee = payees[i].payee
                line.payeeId = payees[i]._id
                if (payees[i].defaultCategory) {
                    if (line.items[0]) {
